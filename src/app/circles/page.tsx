...
export default function CirclesPage() {
  ...
  return <main className="min-h-screen bg-[var(--pact-bg)] pb-28 text-[var(--pact-text)]"><div className="mx-auto max-w-5xl px-5 pb-10 pt-8 md:px-10 md:pt-14">
    ...
    <section className="-mx-5 flex snap-x gap-4 overflow-x-auto px-5 py-6 md:-mx-10 md:px-10" aria-label="Your circles">{isLoading ? <p className="text-sm text-[var(--pact-text-muted)]">Loading circles…</p> : circlesQuery.isError ? <div className="flex w-full flex-col gap-3 py-4 text-sm text-[var(--pact-text-muted)]"><p>We couldn&apos;t load your circles.</p><button type="button" onClick={() => circlesQuery.refetch()} className="w-fit rounded-full border border-[var(--pact-violet)] px-4 py-2 font-semibold text-[var(--pact-text)]">Try again</button></div> : filtered.length === 0 ? <p className="py-4 text-sm text-[var(--pact-text-muted)]">No circles found. Create one to get started.</p> : filtered.map((circle: any) => {
      ...
      return <Link key={circle.id} href={`/circles/${circle.id}`} className="flex aspect-square w-[calc((100%-2rem)/3)] max-w-36 shrink-0 grow-0 basis-[calc((100%-2rem)/3)] snap-start items-center justify-center rounded-full border border-[var(--pact-violet)]/50 bg-[var(--pact-surface)] p-3 text-center transition hover:bg-[var(--pact-surface-2)]" style={{ boxShadow: `0 0 ${glow}px rgba(24,119,242,${glowOpacity})` }}><span className="flex flex-col items-center px-2">{circle.photo_url ? <span className="relative block h-10 w-10 overflow-hidden rounded-full"><Image src={circle.photo_url} alt="" fill sizes="40px" className="object-cover" /></span> : <span className="block text-2xl">{circle.emoji || circle.icon_emoji || '◌'}</span>}<span className="mt-1 block w-full truncate text-xs font-bold leading-tight text-[var(--pact-text)]">{circle.name}</span><span className="mt-1 block text-[10px] font-semibold text-[var(--pact-gold)]">{circle.member_count || 0} active</span></span></Link>
    })}</section>
