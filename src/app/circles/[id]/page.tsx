...
export default function CircleDetailPage() {
  ...
  return <main className="min-h-screen bg-[var(--pact-bg)] pb-24 text-[var(--pact-text)]"><DetailPageHeader title={circle.name || 'Circle'} fallbackHref="/circles" maxWidthClassName="max-w-4xl" /><div className="mx-auto max-w-4xl px-5 pb-12 pt-8">
    ...
    <section className="pt-8"><h2 className="text-xs font-bold uppercase tracking-[0.24em] text-[var(--pact-violet)]">Pacts in this circle</h2>{!isMember ? <p className="py-8 text-sm text-[var(--pact-text-muted)]">Join this circle to view its pacts.</p> : pacts.length ? <div className="mt-4">{pacts.map((pact: any) => { const progress = getPactProgress(pact); return <Link key={pact.id} href={`/pacts/${pact.id}`} className="flex items-center gap-4 border-t border-[var(--pact-hairline)] py-4 hover:border-[var(--pact-violet)]"><PactProgressRing completed={progress.completed} total={progress.total} missed={progress.missed} size={58} /><div className="min-w-0 flex-1"><p className="truncate font-bold text-[var(--pact-text)]">{pact.title}</p><p className="mt-1 text-sm text-[var(--pact-text-muted)]">Day {progress.completed} of {progress.total}</p></div></Link> })}</div> : <p className="py-8 text-sm text-[var(--pact-text-muted)]">No pacts in this circle yet.</p>}</section>
  </div>{circle && qrOpen && <CircleQRFullView circle={circle} pacts={pacts} onClose={() => setQrOpen(false)} />} {circle && <InviteMembersModal isOpen={inviteModal} onClose={() => setInviteModal(false)} circleId={circle.id} circleName={circle.name} existingMemberIds={members.map((m: any) => m.user_id)} />}<ConfirmModal isOpen={leaveModal} onClose={() => setLeaveModal(false)} onConfirm={handleLeave} title="Leave circle?" description={`You'll lose access to ${circle.name}'s pacts until you rejoin.`} confirmLabel="Leave Circle" destructive loading={leaving} /></main>
}
